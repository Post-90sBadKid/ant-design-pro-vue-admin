        ...
        <a-list itemLayout="horizontal" :dataSource="roles">
          <a-list-item slot="renderItem" slot-scope="item, index" :key="index">
            <a-list-item-meta :style="{ marginBottom: '0',display:'flex' }">
              ...
              <a
                slot="title"
                style="text-align: left; display: block; font-size: 16px;"
                @click="selected(item)">{{ item.name }}</a>
            </a-list-item-meta>
    ...
    save () {
      ...
      this.permissions.forEach(menuPermission => {
        ...
        menuPermission.children.forEach(checkboxPermission => {
          if (checkboxPermission.selected.length > 0) {
            permissionIds.push(checkboxPermission.id)
            checkboxPermission.selected.forEach(id => permissionIds.push(id))
          }
        })
      })
