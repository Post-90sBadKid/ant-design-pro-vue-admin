CREATE DATABASE IF NOT EXISTS `ant_design_admin`;

USE  `ant_design_admin`;
...
CREATE TABLE `sys_user` (
  ...
  `nikename` varchar(100) DEFAULT NULL COMMENT '昵称',
  `role_ids` varchar(100) DEFAULT NULL COMMENT '角色列表',
  `enabled` tinyint(1) DEFAULT '1' COMMENT '是否启用',
  `account_non_expired` tinyint(1) DEFAULT '1' COMMENT '帐户是否未过期',
  `account_non_locked` tinyint(1) DEFAULT '1' COMMENT '帐户是否未锁定',
  `credentials_non_expired` tinyint(1) DEFAULT '1' COMMENT '凭据是否未过期',
  PRIMARY KEY (`id`),
